...
function App() {
  ...
  const loadDemoData = async () => {
    try {
      // Load sample ASM files from demo-outputs
      const novemberFiles = ['november_sample_1.json', 'november_sample_2.json', 'november_sample_3.json']
      const julyFiles = ['july_sample_1.json', 'july_sample_2.json', 'july_sample_3.json']
      
      const allData = []
      
      // In production, this would fetch from API
      // For demo, we'll create mock data based on the ASM structure
      const mockData = [
        ...
        { id: 4, instrument: 'Nova FLEX2', sample: 'D_FD_CE_N-5_OPS_CC_01A', date: '2025-07-02', pH: 7.156, pO2: 164.7, pCO2: 33.0, glucose: 4.91, lactate: 1.12, valid: true },
        { id: 5, instrument: 'Nova FLEX2', sample: 'D_FD_CE_N-5_OPS_CC_01B', date: '2025-07-02', pH: 7.095, pO2: 166.5, pCO2: 35.8, glucose: 4.71, lactate: 1.26, valid: true },
        { id: 6, instrument: 'Nova FLEX2', sample: 'D_SD_CE_N-4_OPS_CC_01A', date: '2025-07-02', pH: 7.233, pO2: 205.6, pCO2: 58.8, glucose: 6.52, lactate: 0.11, valid: true },
      ]
              ...
              <Table
                columnDefinitions={[
                  ...
                  },
                  {
                    id: 'pH',
                    header: 'pH',
                    cell: item => item.pH.toFixed(3)
                  },
                  {
                    id: 'pO2',
                    header: 'pO2 (mmHg)',
                    cell: item => item.pO2.toFixed(1)
                  },
                  {
                    id: 'pCO2',
                    header: 'pCO2 (mmHg)',
                    cell: item => item.pCO2.toFixed(1)
                  },
                  {
                    id: 'glucose',
                    header: 'Glucose (g/L)',
                    cell: item => item.glucose.toFixed(2)
                  },
                  {
                    id: 'lactate',
                    header: 'Lactate (g/L)',
                    cell: item => item.lactate.toFixed(2)
                  },
                  {
                    id: 'valid',
                    header: 'Validation',
                    cell: item => item.valid ? <Badge color="green">Valid</Badge> : <Badge color="red">Invalid</Badge>
                  }
                ]}
                items={filteredData}
                loading={loading}
