    ...
    try:
        ...
        if generate_report:
            try:
                ...
                result['certification_report'] = {
                    'available': False,
                    'error': f'Report generation not available: {str(e)}'
                }
...
def validate_with_allotropy(asm_data, validation_level):
    ...
    try:
        ...
        try:
            ...
            print(f"Validation metrics from allotropy: {validation_result.metrics}")
            
            # Extract technique info from ASM manifest or custom fields
            technique = asm_data.get('technique')
            technique_confidence = asm_data.get('technique_confidence')
            
            # If not in custom fields, extract from manifest URL
            if not technique:
                manifest = asm_data.get('$asm.manifest', '')
                if 'solution-analyzer' in manifest or 'solution analyzer' in manifest:
                    technique = 'solution analyzer'
                    technique_confidence = 1.0
                elif 'plate-reader' in manifest:
                    technique = 'plate reader'
                    technique_confidence = 1.0
                elif 'cell-counter' in manifest:
                    technique = 'cell counter'
                    technique_confidence = 1.0
                elif 'hplc' in manifest:
                    technique = 'HPLC'
                    technique_confidence = 1.0
                elif 'mass-spec' in manifest:
